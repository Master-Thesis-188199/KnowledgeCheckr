...
--> statement-breakpoint
ALTER TABLE `Account` ADD CONSTRAINT `fk_account_user1` FOREIGN KEY (`user_id`) REFERENCES `User`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `Answer` ADD CONSTRAINT `fk_Answer_Question1` FOREIGN KEY (`Question_id`) REFERENCES `Question`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `Category` ADD CONSTRAINT `fk_Category_Category1` FOREIGN KEY (`prequisite_category_id`) REFERENCES `Category`(`id`) ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `KnowledgeCheck` ADD CONSTRAINT `fk_KnowledgeCheck_user1` FOREIGN KEY (`owner_id`) REFERENCES `User`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `KnowledgeCheck_Settings` ADD CONSTRAINT `fk_KnowledgeCheck_Settings_KnowledgeCheck1` FOREIGN KEY (`knowledgecheck_id`) REFERENCES `KnowledgeCheck`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX `fk_KnowledgeCheck_user1_idx` ON `KnowledgeCheck` (`owner_id`);--> statement-breakpoint
CREATE INDEX `fk_KnowledgeCheck_Settings_KnowledgeCheck1_idx` ON `KnowledgeCheck_Settings` (`knowledgecheck_id`);--> statement-breakpoint
CREATE INDEX `fk_Question_Category1_idx` ON `Question` (`category_id`);--> statement-breakpoint
CREATE INDEX `fk_Question_KnowledgeCheck1_idx` ON `Question` (`knowledgecheck_id`);--> statement-breakpoint
CREATE INDEX `fk_session_user_idx` ON `Session` (`user_id`);--> statement-breakpoint
CREATE INDEX `fk_user_has_KnowledgeCheck_KnowledgeCheck1_idx` ON `User_contributesTo_KnowledgeCheck` (`knowledgecheck_id`);--> statement-breakpoint
