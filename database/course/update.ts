...
async function updateBaseCourseProperties(db: DrizzleDB, course: Course) {
  ...
  await db.update(db_knowledgeCheck).set(updates).where(eq(db_knowledgeCheck.id, course.id))
}
...
export async function updateCourseShareToken({ courseId, token }: { courseId: Course['id']; token: Course['share_key'] }) {
  ...
  const result = await db
    ...
    .set({ share_key: token })
    .where(and(eq(db_knowledgeCheck.id, courseId), eq(db_knowledgeCheck.owner_id, userId)))
