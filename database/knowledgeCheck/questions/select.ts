...
export default async function getKnowledgeCheckQuestions(db: DrizzleDB, knowledgeCheck_id: KnowledgeCheck['id']) {
  ...
  const raw_questions = await db.select().from(db_question).where(eq(db_question.knowledgecheckId, knowledgeCheck_id)).orderBy(db_question.createdAt)

  for (const question of raw_questions) {
    const answers = await db.select().from(db_answer).where(eq(db_answer.questionId, question.id)).orderBy(db_answer.createdAt)
...
export async function getKnowledgeCheckQuestionById<ExpectedQuestion extends Question>(question_id: Question['id']): Promise<ExpectedQuestion | null> {
  ...
  const answers = await db.select().from(db_answer).where(eq(db_answer.questionId, question_id)).orderBy(db_answer.createdAt)
