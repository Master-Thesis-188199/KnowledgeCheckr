...
const useToolbarNavigation = (toolbarRef: React.RefObject<HTMLDivElement | null>) => {
  ...
  const collectItems = useCallback(() => {
    ...
    return Array.from(toolbarRef.current.querySelectorAll<HTMLElement>('button:not([disabled]), [role="button"]:not([disabled]), [tabindex="0"]:not([disabled])'))
  }, [toolbarRef])
...
export const Toolbar = forwardRef<HTMLDivElement, ToolbarProps>(({ children, className, variant = 'fixed', ...props }, ref) => {
  ...
  return (
    <div ref={composedRef} role='toolbar' aria-label='toolbar' data-variant={variant} className={cn('tiptap-toolbar', className)} {...props}>
      {children}
    </div>
  )
})
...
export const ToolbarSeparator = forwardRef<HTMLDivElement, BaseProps>(({ ...props }, ref) => <Separator ref={ref} orientation='vertical' decorative {...props} />)
