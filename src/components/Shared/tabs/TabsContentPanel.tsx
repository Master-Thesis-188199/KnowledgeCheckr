...
export function TabsContentPanel({ children, tab, className }: { children: React.ReactNode; tab: Tab['name']; className?: string }) {
  ...
  if (!tabs.some((t) => t.name.toLocaleLowerCase().includes(tab.toLowerCase()))) {
    throw new Error(`Tab with name "${tab}" does not exist in the current TabsProvider context.`)
  }

  return (
    <div data-current={currentTab.toLocaleLowerCase().includes(tab.toLowerCase())} className={cn(className, 'data-[current=false]:hidden')}>
      {children}
    </div>
  )
}
