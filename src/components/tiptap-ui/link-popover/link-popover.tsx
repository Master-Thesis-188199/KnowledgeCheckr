...
export const LinkButton = forwardRef<HTMLButtonElement, ButtonProps>(({ className, children, ...props }, ref) => {
  return (
    <Button type='button' className={className} variant='ghost' role='button' tabIndex={-1} aria-label='Link' tooltip='Link' ref={ref} {...props}>
    ...
    <Card
      ...
      <CardBody
        ...
        <CardItemGroup orientation='horizontal'>
          ...
          <ButtonGroup>
            <Button type='button' onClick={setLink} title='Apply link' disabled={!url && !isActive} variant='ghost'>
          ...
          <ButtonGroup>
            <ButtonGroup>
              <Button type='button' onClick={openLink} title='Open in new window' disabled={!url && !isActive} variant='ghost'>
...
export const LinkPopover = forwardRef<HTMLButtonElement, LinkPopoverProps>(
  ({ editor: providedEditor, hideWhenUnavailable = false, onSetLink, onOpenChange, autoOpenOnLinkActive = true, onClick, children, ...buttonProps }, ref) => {
