...
import QuestionSelectionProvider, { useQuestionSelectionContext } from '@/components/check/QuestionSelectionProvider'
...
export default function SingleChoiceQuestion({ id, type, question, points, category, answers }: SingleChoiceQuestion) {
  return (
    <QuestionSelectionProvider maxSelection={1}>
...
function ChoiceOption({ id, answer }: SingleChoiceAnswer & Pick<Question, 'id'>) {
  ...
  return (
    <label className='relative flex items-center gap-4 rounded-md p-3 px-4 ring-1 ring-neutral-200 select-none hover:cursor-pointer has-checked:ring-2 dark:bg-neutral-700/40 dark:ring-neutral-500/80 dark:hover:bg-neutral-700/60 dark:has-checked:bg-neutral-600/80 dark:has-checked:ring-neutral-400/60'>
      <input
        type='checkbox'
        checked={isSelected(answer)}
        onClick={() => toggleSelection(answer)}
        className={twMerge(
          'size-3.5 appearance-none rounded-full ring-2 ring-blue-500 ring-offset-neutral-700 checked:mr-0.5 checked:size-3 checked:bg-neutral-300 checked:ring-2 checked:ring-neutral-300 checked:ring-offset-2 dark:ring-neutral-400',
        )}
      />
      ...
      <div
        className={twMerge(
          'absolute top-0 right-0 rounded-tr-md rounded-bl-lg p-1.5 text-xs dark:bg-neutral-700',
          selection.length >= (maxSelection || 0) ? 'dark:bg-red-700/20 dark:text-red-200/70' : 'bg-blue-500',
          maxSelection === undefined && 'hidden',
        )}>
