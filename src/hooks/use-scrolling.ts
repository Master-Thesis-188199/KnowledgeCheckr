...
export function useScrolling(target?: ScrollTarget, options: UseScrollingOptions = {}): boolean {
  ...
  useEffect(() => {
    ...
    const element: EventTargetWithScroll = target && typeof Window !== 'undefined' && target instanceof Window ? target : ((target as RefObject<HTMLElement>)?.current ?? window)

    // Mobile: fallback to document when using window
    const eventTarget: EventTargetWithScroll = fallbackToDocument && element === window && typeof document !== 'undefined' ? document : element
