...
describe('Verify selection of practice questions by category', () => {
  ...
  it('Verify users can select category in /practice/category page', () => {
    ...
    const randomCategory = dummyCategories.at((Math.random() * dummyCategories.length) % dummyCategories.length)!
    cy.get('#category-selection').children().contains(randomCategory).click()

    cy.url({ timeout: 5 * 1000 }).should('eq', `${baseURL}/checks/${dummyCheck.share_key}/practice?category=${randomCategory}`)
  })
})
